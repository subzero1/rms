...
$(function(){
	$("#nd").change(function(){ 
	navTab.openTab('sgfezb', 'wxdw/sgfezb.do?lb=fezb&nd='+$(this).val(), {title:'项目信息'});
	});
});
	...
	<table class="report" border="0" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">
			...
			<tbody>
				...
				<tr>
					...
					<c:forEach items="${qyList}" var="qy">
						<td style="width: 80px; text-align: center"><c:if test="${not empty dygxMap[zy.name][qy.name] }"><fmt:formatNumber pattern="0.00" value="${not empty fezbMap[zy.name][qy.name].v1 ? fezbMap[zy.name][qy.name].v1 : 0 }"/> %</c:if></td>
					</c:forEach>
				</tr>
				</c:forEach>
			</tbody>
