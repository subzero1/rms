 ...
 * @description:
 * 
 * @class name:com.jfms.thread.ChangeDrawing_Thread
...
public class ChangeDrawing_Thread extends Thread {
			...
			try {
				...
				while(ro.next()){
					try{
						...
						log.error("find error when update drawing [com.jfms.thread.ChangeDrawing_thread.java][5]:" + e);
					}	
				}
				
			} catch (InterruptedException e) {
				log.error("find error when update drawing [com.jfms.thread.ChangeDrawing_thread.java][1]:" + e);
			} catch (Exception e) {
				log.error("find error when update drawing [com.jfms.thread.ChangeDrawing_thread.java][2]:" + e);
			}
		}
	} 
		...
		try{
			...
			while(ro.next()){
				try{
					...
					log.error("find error when update drawing [com.jfms.thread.ChangeDrawing_thread.java][6]:" + e + e.getMessage());
				}
			}
		}
		catch(Exception e){
			log.error("find error when update drawing [com.jfms.thread.ChangeDrawing_thread.java][3]:" + e + e.getMessage());
	...
	private boolean changeDrawingForSingle(Hashtable paramMap) throws Exception{
		...
		try {
			...
			log.error("find error when update drawing [com.jfms.thread.ChangeDrawing_thread.java][4]:" + e + e.getMessage());
