...
public class Dept {
	...
	public ModelAndView deptList(HttpServletRequest request,HttpServletResponse response ,HttpSession session)  {
		...
		List<Ta01_dept> depList = (List<Ta01_dept>)dao.search("from Ta01_dept where area_name in (select area.name from Tc03_area area where area.flag like'%1%') order by area_name,id");//所有部门（树）
