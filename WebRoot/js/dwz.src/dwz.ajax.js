 ...
 * 实现forward两种转向:navTab/dialog(modify at 2011-04-26)
 */
function dialogAjaxDone(json){
	DWZ.ajaxDone(json);
	if (json.statusCode == DWZ.statusCode.ok){
		if (json.navTabId && (json.navTabId != '_current')){
			if ("forward" != json.callbackType && "" != json.forwardUrl) {
				navTab.reload(json.forwardUrl, {navTabId: json.navTabId});
			}else{
				$panel = $(navTab._getPanel(json.navTabId));
				$page_form = $("#pagerForm",$panel);
				if($page_form.length>0){	//翻页列表刷新	 modify at 2011-07-11
					navTabPageBreak({navTabId: json.navTabId}, json.rel);
				}else{
					navTab.reloadFlag(json.navTabId);
				}
			}
		} else if(json.navTabId && (json.navTabId == '_current')){
			if ("forward" != json.callbackType && "" != json.forwardUrl) {
				navTab.reload(json.forwardUrl);
			}else{
				navTab.reload();
...
/**
 * 
 * @param {Object} args {pageNum:"",numPerPage:"",orderField:"",orderDirection:"",formId:""}
 * @param String formId 分页表单选择器，非必填项默认值是 "pagerForm" formId放到args中(modify at 2011-04-28)
 */
function _getPagerForm($parent, args) {
	var formId = "pagerForm";
	if (args["formId"]) formId = args["formId"];
	...
	if (form) {
		...
		if (args["numPerPage"]) {			
			...
			//切换每页记录数时,默认从第一页显示  modify at 2011-07-19
			var temp_v = form[DWZ.pageInfo.numPerPage].value;
			if(args["numPerPage"] != null && args["numPerPage"] != temp_v){
...
function ajaxTodo(url, callback ,data){	//增加data参数 modify at 2011-09-14
...
function uploadifyComplete(event, queueId, fileObj, response, data){
	...
	if("navTab"==json.pannelType){	// 不同面板上传后处理不同 modify at 2011-09-14
		navTabAjaxDone(json);
	}else{
		dialogAjaxDone(json);
	}
