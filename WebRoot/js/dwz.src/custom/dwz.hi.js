/**
 * DWZ 瀹氬埗鍖栧紑鍙� 锛圝-Hi Java蹇�熷紑鍙戝钩鍙帮級
 * 鏁版嵁搴撳叧鑱旇〃: 鏌ユ壘甯﹀洖銆佷富浠�
		...
		removeSelected: function(options){
			...
			return this.each(function(){
				...
				$this.click(function(){
					var ids = getIds();
					if (!ids) {
						alertMsg.error("璇烽�夋嫨瑕佸垹闄ょ殑璁板綍");
						return false;
					}
					alertMsg.confirm($this.attr("title") || "纭疄瑕佸垹闄よ繖浜涜褰曞悧?", {
						okCall: function(){
							$.ajax({
		...
		itemDetail: function(){
			return this.each(function(){
				...
				if (!$table.hasClass('noadd')) {
					var $addBut = $('<div class="button"><div class="buttonContent"><button type="button">增加</button></div></div>').insertBefore($table).find("button");
			...
			function tdHtml(field, itemDetail){
				...
				switch(field.type){
					case'del':
						html = '<a href="javascript:void(0)" class="btnDel">鍒犻櫎</a>';
						break;
					case 'lookup':
						var suggestFrag = '';
						...
						html = '<input type="hidden" name="'+itemDetail+'[#index#].'+field.lookupName+'.id"/>'
							...
							+ '<a class="btnLook" href="'+field.lookupUrl+'" lookupGroup="'+itemDetail+'" lookupName="'+field.lookupName+'" index="#index#" title="鏌ユ壘甯﹀洖">鏌ユ壘甯﹀洖</a>';
						break;
					case 'attach':
						html = '<input type="hidden" name="'+itemDetail+'[#index#].'+field.lookupName+'.id"/>'
							+ '<input type="text" name="'+lookupFieldName+'" size="'+field.size+'" readonly="readonly"/>'
							+ '<a class="btnAttach" href="'+field.lookupUrl+'" lookupGroup="'+itemDetail+'" lookupName="'+field.lookupName+'" index="#index#" width="560" height="300" title="鏌ユ壘甯﹀洖">鏌ユ壘甯﹀洖</a>';
						break;
					case 'enum':
						$.ajax({
